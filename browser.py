...
class LinkedInBrowser:
    ...
    async def scroll_to_load_all_results(self) -> int:
        ...
        Strategy: scroll each <li> into view one by one to force all articles to render.
        Returns the final rendered article count.
        """
        cards_selector = "ol.profile-list article.profile-list-item"
        li_selector = "ol.profile-list > li"

        # Get total <li> count (always present, even before articles render)
        li_count = await self.page.locator(li_selector).count()
        if li_count == 0:
            return 0

        # Scroll each <li> into view to trigger its article to render
        for i in range(li_count):
            ...
            try:
                ...
                pass
            # Brief pause every few items to let rendering catch up
            if (i + 1) % 5 == 0:
        ...
        # Scroll back to top of the results list
        first_li = self.page.locator(li_selector).first
        try:
            await first_li.scroll_into_view_if_needed(timeout=3000)
        except Exception:
