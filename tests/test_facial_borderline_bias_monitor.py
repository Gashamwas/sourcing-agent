...
def test_orchestrator_increments_borderline_counter_under_flag_on_singleton_path():
    ...
    monitor's pre-alias counter exactly once for the candidate's string_id.

    Slice-13 invariants survive: ``_prior_outcomes`` carries FACIAL_YES
    (alias still works), and ``record_decision`` receives the aliased YES.
    """
    with tempfile.TemporaryDirectory() as td:
        ...
        # Slice-13 invariants must still hold.
        ...
        assert recorded.decision == "FACIAL_YES", (
            "record_decision must continue to receive aliased-YES; the "
            "borderline observability path must NOT replace or augment the "
            "alarm-feeding record."
        )
...
def test_orchestrator_borderline_counter_skipped_when_no_bias_monitor():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with patch("linkedin.orchestrator.facial_judge", return_value=_make_borderline_decision()), \
             patch("linkedin.orchestrator.config.LINKEDIN_FACIAL_BORDERLINE_ENABLED", True):
            # Must not raise. Borderline aliases to YES (slice 13), which then
            # falls through to ``_full_evaluate`` (mocked to return None), so
            # the function's return value is None — that is fine; the
            # invariants we care about are (a) no crash and (b) _prior_outcomes
            # carries the aliased YES.
            asyncio.run(p._evaluate_snippet(snippet))

        # Slice-13 invariant still holds even with no bias monitor.
        assert p._prior_outcomes[snippet.profile_url] == "FACIAL_YES"
