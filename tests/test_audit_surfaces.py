...
class TestR2:
    def test_pure_numeric_h1_fails(self) -> None:
        cap = _capture([_fact(tag="h1", text="1990251114")])
    ...
    def test_numeric_with_suffix_fails(self) -> None:
        cap = _capture([_fact(tag="h1", text="1957683706 Clean")])
        results = check_R2_no_raw_ids_in_titles(cap, {})
        assert len(results) == 1
    ...
    def test_filesystem_path_in_title_fails(self) -> None:
        cap = _capture([_fact(tag="h1", text="/Users/sam/foo")])
        results = check_R2_no_raw_ids_in_titles(cap, {})
        assert len(results) == 1
    ...
    def test_paragraph_not_checked(self) -> None:
        ...
        cap = _capture([_fact(tag="p", text="Run #1990251114 finished")])
        results = check_R2_no_raw_ids_in_titles(cap, {})
        assert results == []
...
class TestR9:
    def test_users_path_fails(self) -> None:
        cap = _capture([_fact(text="/Users/sam/Projects/foo")])
