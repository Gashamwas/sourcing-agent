...
- Walks `_scan_authored_briefs` and decorates each with a
  ``brief_id`` from ``linkedin_state_key(path)``.
...
def _isolate_project_root(tmp_path, monkeypatch):
    ...
    Also stub ``linkedin_state_key`` to return a deterministic hash from
    ...
    monkeypatch.setattr(output_paths, "linkedin_state_key", _fake_state_key)
...
def test_aggregate_briefs_decorates_brief_id_via_linkedin_state_key(
    tmp_path: Path,
) -> None:
    ...
    # Stubbed linkedin_state_key returns the path stem, e.g. brief-alpha.
...
def test_aggregate_briefs_decorates_run_metadata_when_runs_exist(
    ...
    brief_id = path.stem  # matches the stubbed linkedin_state_key
