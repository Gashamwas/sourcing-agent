...
- `_progress_kind_for_source("designer")` maps to
  `DESIGNER_BEHANCE_QUERY_KIND` so the workspace progress aggregator
  doesn't silently skip Designer state-dirs.
- `_dispatch_in_process` accepts `source="designer"` for the
  frozen-app fallback.
- `"designer"` lives in `_SOURCES`.
...
from cloris.control_plane import _SOURCES, _progress_kind_for_source
...
def test_designer_in_sources_tuple() -> None:
    assert "designer" in _SOURCES
...
def test_progress_kind_for_designer_returns_behance_query() -> None:
    ...
    state-dirs — otherwise progress lands silently as 0/0."""

    assert _progress_kind_for_source("designer") == DESIGNER_BEHANCE_QUERY_KIND
