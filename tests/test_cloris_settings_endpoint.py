...
def test_settings_governor_renders_constants(client):
    ...
    assert "MAX_SESSIONS_PER_DAY" in names
...
def test_settings_save_destinations_includes_authored_briefs(
    ...
    ``chdir`` so ``linkedin_state_key`` (which opens the relative path
    ...
    monkeypatch.setattr("cloris.api._CONFIG_DIR", config_dir)
    monkeypatch.setattr("cloris.api._PROJECT_ROOT", tmp_path)
