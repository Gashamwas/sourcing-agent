...
- ``linkedin_state_key()`` reads ``source_config.linkedin.project_id``
...
# ---------------------------------------------------------------------------
# linkedin_state_key fallback
# ---------------------------------------------------------------------------


def test_linkedin_state_key_uses_source_config_path(tmp_path: Path) -> None:
    ...
    key = linkedin_state_key(brief_path=str(brief_path))
    assert "new_value" in key


def test_linkedin_state_key_falls_back_to_flat_field(tmp_path: Path) -> None:
...
def test_linkedin_state_key_stable_across_migration(tmp_path: Path) -> None:
    ...
    legacy_payload["linkedin_project_id"] = "1990251114"
    ...
    migrated_payload["source_config"] = {
        "linkedin": {"project_id": "1990251114"}
    }
    migrated_path = tmp_path / "migrated" / "brief.json"
    _write_brief(migrated_path, migrated_payload)

    legacy_key = linkedin_state_key(brief_path=str(legacy_path))
    migrated_key = linkedin_state_key(brief_path=str(migrated_path))
...
def test_linkedin_blocker_returns_none_when_configured(tmp_path: Path) -> None:
    ...
    payload["source_config"] = {"linkedin": {"project_id": "1990251114"}}
    brief_path = tmp_path / "brief.json"
    _write_brief(brief_path, payload)
...
def test_linkedin_blocker_passes_with_legacy_flat_field(tmp_path: Path) -> None:
    ...
    the same fallback as linkedin_state_key."""
...
def test_github_blocker_always_returns_none(tmp_path: Path) -> None:
    """GitHub has no per-brief destination concept in F2 — the blocker
    is always None. F2's punt; revisit when GitHub gains list/team
    semantics."""
...
def _seed_brief(tmp_path: Path, *, with_linkedin_dest: bool) -> str:
    ...
    return linkedin_state_key(brief_path=str(config_dir / "brief.json"))
...
def test_launch_endpoint_blocks_without_force_when_destination_missing(
    ...
    def fake_spawn(*, source: str, brief_path: Path, mode: str) -> _SpawnResult:
    ...
    monkeypatch.setattr(cloris_api, "_spawn_worker_for_source", fake_spawn)
