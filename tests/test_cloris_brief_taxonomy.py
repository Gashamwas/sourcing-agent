...
def _entry(
    *, kind: str, status: str | None = None, source: str = "linkedin"
) -> StateDirEntry:
...
class TestBriefCountsRollUp:
    ...
    def test_paused_status_counts_as_paused_and_active(self) -> None:
        counts = _compute_brief_counts([
            _entry(kind="authored_brief", status="interrupted"),
            _entry(kind="authored_brief", status="governor_limit_reached"),
        ])
    ...
    def test_abandoned_and_error_count_as_lost(self) -> None:
        counts = _compute_brief_counts([
    ...
    def test_mixed_population(self) -> None:
        ...
        # 1 running + 2 paused = 3 active, of which 1 is working.
