...
- `cloris.control_plane.aggregate_workspace` discovers a researcher
  state_dir under `output/state/researcher/<key>/` (because Slice 1
  widened `_SOURCES` to include `"researcher"`).
...
def test_workspace_aggregator_discovers_researcher_state_dir(tmp_path: Path) -> None:
    ...
    by the workspace aggregator (Slice 1 widened _SOURCES).
    """
    ...
    assert workspace is not None, (
        "Workspace aggregator must discover researcher state dirs after "
        "Slice 1's _SOURCES widening at cloris/control_plane.py:198."
    )
