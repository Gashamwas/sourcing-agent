...
def _populated_v2_raw() -> dict:
    ...
    raw = _minimal_v2_raw()
    raw.update({
        "role_level": "L7",  # senior — exercises senior-role hydration too
        ...
        "senior_role_titles": ["Head of Marketing", "VP Marketing"],
        "senior_role_paradigms": ["release strategy", "awards positioning"],
...
def test_v2_brief_has_calibration_fields_with_safe_defaults():
    ...
    assert nb.senior_role_function_name == ""
...
def test_v2_loader_hydrates_calibration_onto_both_briefs():
    ...
    # _new_brief gets the full set, including the senior-role trio.
    ...
    assert nb.canonical_company_patterns == ["A24", "Neon", "Searchlight"]
    ...
    assert nb.senior_role_titles == ["Head of Marketing", "VP Marketing"]
    assert nb.senior_role_paradigms == ["release strategy", "awards positioning"]
    assert nb.senior_role_function_name == "marketing function"

    # Compat brief mirrors the strategy-relevant set (NOT the senior-role trio).
...
def test_compat_brief_does_not_mirror_senior_role_fields():
    """Senior-role fields stay schema-only — they must NOT leak onto compat Brief."""
    raw = _populated_v2_raw()
    compat = _load_v2_brief(raw)
    for forbidden in ("senior_role_titles", "senior_role_paradigms", "senior_role_function_name"):
        assert not hasattr(compat, forbidden), (
            f"compat Brief unexpectedly carries {forbidden}; senior-role trio is schema-only"
        )
...
def test_v2_loader_warns_when_calibration_fields_missing(caplog):
    """Loading a V2 brief without calibration fields emits a single summary warning."""
    raw = _minimal_v2_raw()
    ...
    assert len(warnings) == 1, "expected exactly one summary calibration warning"
    ...
    for required in (
        ...
        assert required in msg, f"warning summary missing field {required!r}"
    # Senior-role fields are NOT required for an L5 brief.
    assert "senior_role_titles" not in msg
    assert "senior_role_function_name" not in msg
    # Compat brief still loaded successfully.
    assert isinstance(compat, CompatBrief)


def test_v2_loader_warns_on_senior_role_fields_for_senior_brief(caplog):
    """Senior-role fields are required only when is_senior_role()."""
    raw = _minimal_v2_raw()
    raw["role_level"] = "L7"  # promote to senior
    with caplog.at_level(logging.WARNING, logger="shared.brief_loader"):
        _load_v2_brief(raw)
    msg = " ".join(rec.getMessage() for rec in caplog.records if rec.levelno == logging.WARNING)
    assert "senior_role_titles" in msg
    assert "senior_role_paradigms" in msg
    assert "senior_role_function_name" in msg


def test_v2_loader_does_not_warn_when_calibration_fully_populated(caplog):
    """A fully-populated V2 brief produces no calibration warning."""
    raw = _populated_v2_raw()
    with caplog.at_level(logging.WARNING, logger="shared.brief_loader"):
        _load_v2_brief(raw)
    cal_warnings = [
...
def test_validator_does_not_raise_on_missing_fields():
    """Stage-0 is warning-only — must NEVER raise."""
    raw = _minimal_v2_raw()
    compat = _load_v2_brief(raw)
