...
- C2 is structural-only: the orchestrator (slice 13/14) still aliases
  FACIAL_BORDERLINE -> FACIAL_YES at the persistence boundary, so no code
  path increments facial_borderline_count today.
"""
...
# ---------------------------------------------------------------------------
# 5. Orchestrator stays binary today (slice 13 invariant survives C2)
# ---------------------------------------------------------------------------


def test_orchestrator_does_not_increment_borderline_counter_today():
    """Slice 13 invariant: orchestrator's stats dict must not carry a
    `facial_borderline` key, even with the C2 counter widening landed.

    Today, FACIAL_BORDERLINE returned by the parser is aliased to FACIAL_YES
    at the persistence boundary (slice 13). The orchestrator's counter
    increments mirror that translation: `facial_yes` ticks for borderline
    decisions; `facial_borderline` is never created.

    C2 is structural-only at the schema layer; it does not introduce a new
    orchestrator counter. When/if a future slice removes the alias, an
    orchestrator-level counter increment can be added in tandem with that
    change.
    """
    with tempfile.TemporaryDirectory() as td:
        ...
        assert p.stats.get("facial_yes") == 1
