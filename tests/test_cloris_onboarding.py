...
- ``POST /api/onboarding/credential`` handles the success path,
  rejects unknown keys (HTTP 422), and rejects empty values (HTTP 422).
...
def test_post_onboarding_credential_success(
    ...
    env_content = (user_data_dir / ".env").read_text()
    assert "ANTHROPIC_API_KEY=sk-ant-via-api" in env_content
