...
def test_linkedin_progress_bootstrap_ignores_mismatched_projection_file(tmp_path):
    pipeline = _make_linkedin_pipeline(str(tmp_path))
    pipeline.search_config = {
        "strings": [
            {"id": 1, "name": "builders", "boolean": "(ml OR ai)"},
        ]
    }
    ...
    progress = pipeline._load_or_create_progress()

    assert pipeline._runtime_run_id is not None
    assert [search_string.id for search_string in progress.strings] == [1]
    projected = json.loads((tmp_path / "progress.json").read_text())
    assert projected["brief_name"] == "test"
    assert [search_string["id"] for search_string in projected["strings"]] == [1]
