...
- ``POST /api/launch/{source}`` accepts ``{brief_id, mode, force?}``;
...
def client(
    ...
    def fake_spawn(*, source: str, brief_path: Path, mode: str) -> _SpawnResult:
        captures["calls"].append(
            {"source": source, "brief_path": str(brief_path), "mode": mode}
        )
        return _SpawnResult(
            pid=42424,
    ...
    monkeypatch.setattr(cloris_api, "_readiness_blockers", fake_blockers)
