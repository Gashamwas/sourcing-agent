...
def test_smoke_e2e_real_brief_file_lands_saves_on_workspace(
    ...
    The workspace aggregator iterates `_SOURCES` and looks for
    state dirs at `state_root/<source>/<state_key>/`, so the test
    nests the runtime DB under that source-rooted layout.
    """
