...
def test_external_research_skips_incomplete_explicit_linkedin_run(tmp_path):
    brief_path = tmp_path / SOURCE_BRIEF.name
    write_json(brief_path, read_json(SOURCE_BRIEF))
    output_dir = resolve_linkedin_state_dir(brief_path=brief_path, state_dir=tmp_path / "output" / "state" / "linkedin" / "1957683706")
...
def test_finalize_run_snapshot_creates_run_manifest_and_research_input(tmp_path):
    ...
    state_dir = resolve_linkedin_state_dir(
        brief_path=brief_path,
        state_dir=tmp_path / "output" / "state" / "linkedin" / "1957683706",
    )
    _write_linkedin_inputs(
        state_dir,
        brief_path,
...
def test_import_legacy_run_snapshot_filters_unrelated_project_artifacts(tmp_path):
    ...
    write_json(legacy_output_dir / "bias_monitor-1957683706.json", {"decisions": 42})
    ...
    (legacy_output_dir / "candidate_history-1957683706.jsonl").write_text("{}\n")
    (legacy_output_dir / "candidate_history-999999.jsonl").write_text("{}\n")
    (legacy_output_dir / "noise_discoveries-1957683706.jsonl").write_text("{}\n")
    ...
    assert (run_dir / "search_memory-1957683706.json").exists()
    assert (run_dir / "bias_monitor-1957683706.json").exists()
    assert (run_dir / "candidate_history-1957683706.jsonl").exists()
    assert (run_dir / "noise_discoveries-1957683706.jsonl").exists()
...
def test_backfill_from_shared_legacy_dir_uses_run_scoped_runtime_payloads(tmp_path):
    ...
    legacy_output_dir = tmp_path / "output"
    write_json(brief_path, read_json(FDE_BRIEF))
    ...
    with store.connect() as conn:
        conn.execute(
            ...
            VALUES (?, 'linkedin', '1990251114', 'linkedin_string', '2', 'Older lane', 1, 'done',
        ...
        conn.execute(
            ...
            VALUES (?, 'linkedin', '1990251114', 'linkedin_string', '5', 'Newer lane', 1, 'done',
...
def test_perplexity_backend_packages_deep_research_into_external_research_result(
    ...
    batch = MarketEvidenceBatch(
        run_ref="linkedin:output/runs/linkedin/1990251114/run-2",
        source="linkedin",
        output_dir="/tmp/run",
        brief_version="1.3",
        generated_at="2026-04-08T04:43:28.957197+00:00",
                ...
                "hidden_pool_risk_signals": [
                    {
                        ...
                        "supporting_run_refs": ["linkedin:output/runs/linkedin/1990251114/run-2"],
                    }
                ]
            },
            "report_analysis": {},
        ...
        "output": [
            {
                "type": "search_results",
                "results": [
                    {"title": "OpenAI FDE", "url": "https://example.com/openai-fde"},
                ],
            },
            {
                "type": "fetch_url_results",
...
def test_normalize_critic_result_recovers_invalid_keep_sections_from_fallback():
    ...
    assert "Recovered from invalid critic keep_sections output" in " ".join(
        result.section_generation_metadata["market_thesis"]["notes"]
    )
...
def test_adapt_after_block_includes_market_intel_advisory_context(monkeypatch):
    ...
    assert "## Market Intel Advisory Context" in captured["system"]


def test_finalize_run_snapshot_auto_updates_market_intel(monkeypatch, tmp_path):
    pipeline = Pipeline.__new__(Pipeline)
    pipeline._runtime_run_id = 7
    pipeline.brief_path = str(tmp_path / "brief.json")
    pipeline.state_dir = tmp_path / "state"
    pipeline.log_path = tmp_path / "state" / "run_log.jsonl"
    pipeline.log_path.parent.mkdir(parents=True, exist_ok=True)
    pipeline.log_path.write_text("")

    run_dir = tmp_path / "output" / "runs" / "linkedin" / "brief" / "2026-04-08T12-00-00Z__run-7"
    run_dir.mkdir(parents=True, exist_ok=True)
    write_json(tmp_path / "brief.json", read_json(SOURCE_BRIEF))

    class _Artifact:
        artifact_version = 1
        market_identity = SimpleNamespace(market_key="head_ai_lab__nyc__l8_l9")

    monkeypatch.setattr(
        "market_intelligence.run_snapshots.finalize_run_snapshot",
        lambda **_: run_dir,
    )
    monkeypatch.setattr(
        "market_intelligence.engine.update_market_intel",
        lambda **_: _Artifact(),
    )

    pipeline._finalize_run_snapshot()
