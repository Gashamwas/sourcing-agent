...
function is the unit under test, and OSSF lookup uses the shipped
snapshot.
"""
...
def test_niche_unknown_project_scores_low(
    ...
    """An unknown project with no signal returns a niche band."""

    client = _make_client(repo=None, contributors=None, releases=None)
    monkeypatch.setattr(
        "github.network_dependents.fetch_dependents_count",
...
def test_unavailable_signals_normalize_against_available_weights(
    ...
    # rust-lang/rust scores ~0.97 in the shipped snapshot.
