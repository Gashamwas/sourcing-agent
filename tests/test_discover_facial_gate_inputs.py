...
def test_run_brief_id_filter_smoke(tmp_path, capsys):
    ...
    rc = disc.run(_ns(brief_id="1957"), runs_root=runs_root)
    assert rc == 0
    out = capsys.readouterr().out
    assert "total_runs_discovered     : 1" in out
    assert "1957683706" in out
    assert "1990251114" not in out.split("Recommended next step", 1)[0]
