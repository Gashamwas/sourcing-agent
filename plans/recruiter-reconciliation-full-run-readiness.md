...
- [x] Slice 3 — P0: observability summary fields. Touch `github/recruiter_identity_report.py`; update `tests/test_recruiter_identity_report.py`. Backfill: re-emit summary on existing `recruiter-reconciliation-live-dryrun-nyc/` artifacts by re-running the writer (no re-run of the browser flow).
