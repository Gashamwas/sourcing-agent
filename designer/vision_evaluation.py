...
Designer Slice 5. The load-bearing differentiation: Gemini 2.5 Pro
...
def assemble_vision_evaluation_system_prompt(brief: dict[str, Any]) -> str:
    """Build the Gemini 2.5 Pro system prompt for vision evaluation.
...
# Pricing per spec §4.4 — Gemini 2.5 Pro: $1.25/1M input, $10/1M
# output. Hardcoded for the per-call cost estimate; revisit when
# Google adjusts pricing.
GEMINI_2_5_PRO_INPUT_USD_PER_1M_TOKENS = 1.25
GEMINI_2_5_PRO_OUTPUT_USD_PER_1M_TOKENS = 10.0
GEMINI_2_5_PRO_TOKENS_PER_IMAGE = 258  # spec §4.1 measurement
    ...
    *,
    ...
    try:
        ...
        return _fallback_result(
            model=model,
            asset_references=asset_references,
            fallback_reason=f"llm_raise:{exc.__class__.__name__}",
        )
    ...
    # Layer 3: anchor consistency. Doesn't fully fail; instead marks
    # individual principles. Keeps the candidate in the eval surface
    # but flags the per-principle drift.
    anchor_pass_map = _layer3_anchor_consistency(
        raw, rubric_principles=rubric_principles
    )

    # Layer 4: hard-reject patterns (auto-reject, NOT a fallback).
    layer4 = _layer4_hard_reject(raw, hard_reject_patterns=hard_reject_patterns)
    if layer4 is not None:
        # Hard-reject still produces a structured judgment so the
        # workspace surface can render the rejection rationale —
        # but the overall_verdict is forced to "no" and confidence
        # is forced to 1.0 (Cloris is sure this is a no).
    ...
    # All four guards cleared → ship the structured judgment.
    principles = _principles_from_raw(raw, anchor_pass_map=anchor_pass_map)
    cost = _estimate_cost(image_count=len(asset_references))
...
def _estimate_cost(*, image_count: int) -> float:
    """Approximate USD cost for one Gemini 2.5 Pro vision call.
    ...
    estimated_input_tokens = image_count * GEMINI_2_5_PRO_TOKENS_PER_IMAGE + 1500
    estimated_output_tokens = 500

    input_cost = estimated_input_tokens * GEMINI_2_5_PRO_INPUT_USD_PER_1M_TOKENS / 1_000_000
    output_cost = estimated_output_tokens * GEMINI_2_5_PRO_OUTPUT_USD_PER_1M_TOKENS / 1_000_000
