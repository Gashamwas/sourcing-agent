...
def _progress_kind_for_source(source: str) -> str:
    """Which work_unit kind to summarize per source.

    LinkedIn organizes work as ``linkedin_string`` (one work_unit per
    Boolean); GitHub as ``github_query``; Designer as
    ``designer_behance_query`` (the dominant work-unit kind — the
    Behance-discovery channel; CSE channel work-units are aggregated
    elsewhere in the projection layer). Anything else falls back to
    None — callers skip work_unit_progress for that state dir.
    """

    if source == "linkedin":
        return "linkedin_string"
    if source == "github":
        return "github_query"
    if source == "designer":
        return "designer_behance_query"
    return ""
...
_SOURCES: tuple[str, ...] = ("linkedin", "github", "researcher", "designer", "exec_search")
...
def _compute_brief_counts(entries: list[StateDirEntry]) -> BriefCounts:
    ...
    for e in entries:
        ...
        if status == "running":
            working += 1
        elif status in ("interrupted", "governor_limit_reached", "paused"):
            paused += 1
        elif status in ("completed", "succeeded"):
            finished += 1
        elif status in ("abandoned", "error"):
            lost += 1
        else:
            # Unknown / null status on an authored brief — bucket with
            # paused so the user still sees it as something to attend to.
            paused += 1
    return BriefCounts(
    ...
    for source, state_dir in enumerate_state_dirs(state_root):
        ...
        if runtime_state_present and not runtime_state_corrupt:
            ...
            if latest_run_id is not None:
                ...
                progress_kind = _progress_kind_for_source(source)
                if progress_kind:
...
def aggregate_run_report(
    ...
    *,
    ...
    # run report agree on the work-unit picture.
    progress_kind = _progress_kind_for_source(source)
    progress = (
        read_models.work_unit_progress(
        ...
        if progress_kind
        else read_models.WorkUnitProgress(kind="not_found")
    )
...
def aggregate_briefs(
    *,
    ...
    ``linkedin_state_key(path)``, the same hash :func:`start_run`
    ...
    from cloris.api import _CONFIG_DIR, _scan_authored_briefs
    ...
    for brief in raw:
        try:
            brief_id = linkedin_state_key(brief_path=str(brief.path))
        except Exception:
...
def aggregate_workspace(
    *,
    ...
    # brief, then build cross-source links per card. The resolve call
    # is idempotent — re-runs on already-linked candidates are no-ops.
    ...
    try:
        ...
        for person in persons:
            ...
            primary_pkey = (primary.source, primary.state_key, primary.candidate_id)
        ...
        # Identity-resolution layer failures must never break the
        # workspace render — F6 is additive on top of the C-bis
        # source-aware shape.
        cross_links_by_pkey = {}
        secondary_pkeys = set()
