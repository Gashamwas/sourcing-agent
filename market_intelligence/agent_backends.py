...
def _normalize_critic_result(
    ...
    for section in (
        ...
        raw_section = raw_keep_sections.get(section)
        fallback_section = fallback.keep_sections.get(section, [])
        if section not in raw_keep_sections:
            if fallback_section:
                sanitized_keep_sections[section] = fallback_section
                repaired_sections.add(section)
            continue
        if isinstance(raw_section, list) and raw_section and not sanitized_keep_sections.get(
            section
        ):
            if fallback_section:
                sanitized_keep_sections[section] = fallback_section
                repaired_sections.add(section)
        elif not isinstance(raw_section, list) and fallback_section:
            sanitized_keep_sections[section] = fallback_section
            repaired_sections.add(section)
