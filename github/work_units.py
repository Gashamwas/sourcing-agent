...
class GitHubWorkUnitService:
    ...
    def load_or_create_progress(self, *, resume: bool = False):
        ...
        pipeline._seen_usernames = set(progress.discovered_usernames)
    ...
    def save_progress(self) -> WorkUnitCheckpoint | None:
        ...
        if not pipeline._progress:
            return None
        pipeline._progress.discovered_usernames = list(pipeline._seen_usernames)
        if pipeline._client:
    ...
    def dedup_usernames(self, usernames: list[str]) -> list[str]:
        ...
        for username in usernames:
            if (
                ...
                and username not in pipeline._seen_usernames
        ...
        if new_queries:
            ...
            if pipeline._runtime_run_id:
                for seed in seeds:
                    ...
                    if username:
                        pipeline._runtime_state.mark_graph_expansion_seed_processed(
