...
async def run_day_cycle(
    ...
    def _signal_handler(sig, frame):
        _print_governor("Shutdown signal received. Finishing current activity...")
        stop_event.set()
    ...
    session_num = 0
    resume = False  # First session is fresh; subsequent sessions resume
    ...
    while not stop_event.is_set():
        ...
        _print_governor(
            ...
            f"Reason: {result['shutdown_reason']}"
        )

        # After first session, subsequent sessions should resume
        resume = True
