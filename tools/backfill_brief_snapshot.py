...
3. For each such run:
   a. Find the brief in `config/` whose `linkedin_state_key()` /
...
def _build_brief_index(config_dir: Path) -> dict[tuple[str, str], Path]:
    ...
    `linkedin_state_key`) AND a GitHub brief_id (via
    `github_state_key`); we register it under both.
    """

    from shared.output_paths import github_state_key, linkedin_state_key

    index: dict[tuple[str, str], Path] = {}
    for brief_path in _scan_briefs(config_dir):
        try:
            li_key = linkedin_state_key(brief_path=str(brief_path))
